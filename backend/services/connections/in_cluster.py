"""
In-Cluster Connection Implementation

Uses cluster-manager gRPC service for Kubernetes API access.
This is used when Flowfish is deployed in the same cluster it monitors.
"""
...
class InClusterConnection(ClusterConnection):
    """
    Connection for in-cluster deployment.
    
    Uses cluster-manager gRPC service which has ClusterRole permissions
    to access Kubernetes API from within the cluster.
    """
    ...
    async def _do_connect(self) -> None:
        """Verify cluster-manager is accessible"""
        # gRPC client connects lazily, just verify it's available
        health = await self._grpc_client.health_check()
        if not health.get("healthy"):
            raise ConnectionError(f"cluster-manager not healthy: {health.get('message')}")
    
    async def _do_disconnect(self) -> None:
        """No explicit disconnect needed for gRPC client"""
        pass
    
    async def get_cluster_info(self) -> ClusterInfo:
        """Get cluster info via cluster-manager gRPC"""
        self.mark_used()
        result = await self._grpc_client.get_cluster_info(cluster_id=str(self.cluster_id))
    ...
    async def check_gadget_health(self) -> GadgetHealth:
        """Check gadget health via cluster-manager gRPC"""
        self.mark_used()
        ...
        return GadgetHealth(
            ...
            error=result.get("error"),
            details={
                "ebpf_capable": details.get("ebpf_capable", False),
                "total_restarts": details.get("total_restarts", 0),
                "issues": details.get("issues", [])
            }
        )

    async def check_beyla_health(self, beyla_namespace: str = "") -> BeylaHealth:
        """Check Beyla + L7 Collector health via dedicated cluster-manager gRPC call."""
        self.mark_used()
    ...
    async def get_namespaces(self) -> List[Namespace]:
        """Get namespaces via cluster-manager gRPC"""
        self.mark_used()
        result = await self._grpc_client.list_namespaces(cluster_id=str(self.cluster_id))
    ...
    async def get_deployments(self, namespace: Optional[str] = None) -> List[Deployment]:
        """Get deployments via cluster-manager gRPC"""
        self.mark_used()
    ...
    async def get_pods(self, namespace: Optional[str] = None, label_selector: Optional[str] = None) -> List[Pod]:
        """Get pods via cluster-manager gRPC"""
        self.mark_used()
    ...
    async def get_services(self, namespace: Optional[str] = None) -> List[Service]:
        """Get services via cluster-manager gRPC"""
        self.mark_used()
    ...
    async def get_configmaps(self, namespace: Optional[str] = None) -> List[ConfigMap]:
        """Get configmaps via cluster-manager gRPC"""
        self.mark_used()
        result = await self._grpc_client.list_configmaps(
            cluster_id=str(self.cluster_id), namespace=namespace or ""
        )
        return [ConfigMap(
            name=cm.get("name", ""),
    ...
    async def get_secrets(self, namespace: Optional[str] = None) -> List[Secret]:
        """Get secrets via cluster-manager gRPC"""
        self.mark_used()
    ...
    async def get_statefulsets(self, namespace: Optional[str] = None) -> List[StatefulSet]:
        """Get statefulsets via cluster-manager gRPC"""
        self.mark_used()
    ...
    async def get_network_policies(self, namespace: Optional[str] = None) -> List:
        """Get network policies via cluster-manager gRPC"""
        self.mark_used()
    ...
    async def get_ingresses(self, namespace: Optional[str] = None) -> List:
        """Get ingresses via cluster-manager gRPC"""
        self.mark_used()
    ...
    async def get_routes(self, namespace: Optional[str] = None) -> List:
        """Get OpenShift routes via cluster-manager gRPC"""
        self.mark_used()
    ...
    async def get_labels(self, namespace: Optional[str] = None) -> List[str]:
        """Get labels via cluster-manager gRPC"""
        self.mark_used()
