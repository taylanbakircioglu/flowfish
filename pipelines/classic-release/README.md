...
| `OPENSHIFT_NAMESPACE` | Deployment namespace | `pilot-flowfish` | No |
| `STORAGE_CLASS` | Storage class name | `your-storage-class` | No |
| `DOMAIN_NAME` | Base domain | `company.com` | No |
| `FRONTEND_URL` | Frontend URL | `pilot-flowfish.company.com` | No |
| `API_BASE_URL` | API base URL | `pilot-flowfish.company.com/api` | No |
| `TLS_SECRET_NAME` | TLS secret name | `wildcard-tls` | No |
| `POSTGRES_HOST` | PostgreSQL host | `postgresql.pilot-flowfish.svc` | No |
| `POSTGRES_USER` | PostgreSQL username | `flowfish` | No |
| `POSTGRES_PASSWORD` | PostgreSQL password | `********` | **Yes** |
| `REDIS_HOST` | Redis host | `redis.pilot-flowfish.svc` | No |
| `REDIS_PASSWORD` | Redis password | `********` | **Yes** |
| `CLICKHOUSE_HOST` | ClickHouse host | `clickhouse.pilot-flowfish.svc` | No |
| `CLICKHOUSE_USER` | ClickHouse username | `flowfish` | No |
| `CLICKHOUSE_PASSWORD` | ClickHouse password | `********` | **Yes** |
| `RABBITMQ_HOST` | RabbitMQ host | `rabbitmq.pilot-flowfish.svc` | No |
...
| `NEO4J_HOST` | Neo4j host | `neo4j.pilot-flowfish.svc.cluster.local` | No |
...
**Verify:**
```bash
oc get secret example.com -n pilot-flowfish
```
...
```bash
# Grant anyuid SCC
oc adm policy add-scc-to-user anyuid system:serviceaccount:pilot-flowfish:default

# Grant privileged SCC (only if required)
oc adm policy add-scc-to-user privileged system:serviceaccount:pilot-flowfish:default
```
...
1. Check if images exist in Harbor:
   ```bash
   oc describe pod <pod-name> -n pilot-flowfish
   ```

2. Check image pull secrets:
   ```bash
   oc get secrets -n pilot-flowfish
   ```

3. Check pod logs:
   ```bash
   oc logs <pod-name> -n pilot-flowfish
   ```
...
3. Verify database pods are running:
   ```bash
   oc get pods -n pilot-flowfish -l tier=infrastructure
   ```
...
1. Check backend pod logs:
   ```bash
   oc logs deployment/backend -n pilot-flowfish
   ```

2. Check if all dependencies are ready:
   ```bash
   oc get pods -n pilot-flowfish
   ```

3. Check service endpoints:
   ```bash
   oc get endpoints -n pilot-flowfish
   ```
...
oc rollout undo deployment/backend -n pilot-flowfish
oc rollout undo deployment/frontend -n pilot-flowfish

# Check rollout history
oc rollout history deployment/backend -n pilot-flowfish
```
